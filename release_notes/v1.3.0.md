...
### Power Management
- WiFi power save mode now dynamically responds to AC/battery status
- Desktop systems always use performance mode (ignores device batteries)
- Battery timeout protection prevents hangs from stuck indicators

### Dispatcher Stability  
- Removed race condition with systemd service
- Interface filtering prevents errors on virtual devices
- Link speed retry logic handles wake-from-sleep
- Timeout protection prevents battery read hangs

### Installation & Upgrades
- Legacy install detection works correctly
- Version comparison handles pre-1.3.0 installs
- Uninstaller doesn't break SteamOS read-only filesystem
- Post-install optimization prompt for immediate apply

### Disabled Features
- `--force-performance` temporarily disabled (broken, scheduled for v1.4.0)
...
```bash
# Decision tree:
1. No system battery (BAT0/BAT1) → Desktop → Performance mode
2. Has system battery → Check power supplies:
   - type=Mains online=1 → AC connected → Performance
   - type=USB online=1   → USB-C charging → Performance  
   - Fallback: Battery status=Charging/Full → Performance
   - Otherwise → Battery mode → Power save
```

### Supported Platforms
- Steam Deck (USB-C charging via `type=USB`)
- Bazzite laptop (AC adapter via `type=Mains`)
- SteamOS desktop (performance mode)
- Bazzite desktop (ignores device batteries)
