...
### 1. **Breathing CAKE - Adaptive QoS**
Revolutionary dynamic bandwidth management that eliminates bufferbloat without sacrificing throughput.

**How it works:**
- **Median Filter:** 5-sample rolling window removes bandwidth outliers/spikes
- **Dual-Stage Smoothing:** Median → EMA (alpha 0.1) for buttery-smooth transitions
- **Direction-Based Hysteresis:** 3-tick (6 second) stability requirement before CAKE updates
- **Intelligent Thresholds:** Only applies changes when bandwidth shifts by 25 Mbit or 20%
- **Optimal Bandwidth:** Uses 85% of link capacity for perfect bufferbloat/throughput balance

**Result:** CAKE qdisc adapts to your connection without causing latency spikes or jitter.
...
**CAKE Bandwidth Smoothing:**
- Median filter (10-second rolling window) eliminates outliers
- EMA alpha reduced from 0.3 → 0.1 (90% weight to historical data)
- Hysteresis prevents flip-flopping between bandwidth targets
- Higher change thresholds (5→25 Mbit, 10%→20%) ignore minor fluctuations
...
### 6. **Backend Tuning (iwd/wpa_supplicant)**
...
- Auto-installs Rust toolchain (rustup) if missing
- Detects missing `cc` linker and installs `base-devel` via pacman
- Handles `steamos-readonly disable` automatically
...
### **Critical Fixes:**
1. **CAKE bandwidth jitter** - Median + EMA smoothing eliminates rapid changes
2. **Power save flapping** - Hysteresis prevents AC/battery state thrashing
3. **Band steering loops** - Hysteresis stops AP ping-ponging
4. **Text parsing brittleness** - DBus provides structured data
5. **CPU overhead** - Async I/O eliminates constant polling
6. **Race conditions** - Stateful architecture prevents conflicting updates

### **Installation Fixes:**
1. **SteamOS linker loops** - Auto-installs `base-devel` package
2. **PATH issues with sudo** - Uses absolute paths for all binaries
3. **"Text file busy" errors** - Stops service before upgrade
4. **Missing cargo** - Uses direct file path checks
5. **Broken Rust installs** - Repairs via `rustup self update`
